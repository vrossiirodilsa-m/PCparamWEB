...
<body id="body">
    <div class="header-container">
        ...
        <div class="nav-buttons">
            ...
            <button class="theme-btn" onclick="toggleTheme()" id="theme-toggle-btn">Тема</button>
        </div>
    </div>
    ...
    <script>
        ...
        function applyTheme(theme) {
            ...
            localStorage.setItem('pcparam_theme', theme);
        }
        ...
        const savedTheme = localStorage.getItem('pcparam_theme');
        if (savedTheme) {
        ...
        function updateStats() {
            fetch('/api/stats')
                ...
                .then(data => {
                    ...
                    // Текстовая строка со стилизацией оранжевой температуры
                    ...
                    document.getElementById('net-line').innerHTML = 'NET:   <span style="color: #03A9F4;">↓</span> ' + downStr + '   <span style="color: #4CAF50;">↑</span> ' + upStr;
                    ...
                    drawDualGraph('net-canvas', historyData.netDown, historyData.netUp, '#03A9F4', '#4CAF50');
                })
        ...
        function loadConfigAndApply() {
            fetch('/api/config')
                .then(res => res.json())
                .then(cfg => {
                    if (cfg.theme && !localStorage.getItem('pcparam_theme')) {
